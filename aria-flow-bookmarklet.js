(async function installAriaFlowBookmarklet() {
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      #${MODAL_ID} .user { background: #eef6ff; border-color: #b2ddff; }
      #${MODAL_ID} .assistant { background: #fff; border-left: 4px solid #d0d5dd; }
      #${MODAL_ID} .api-tool { border-color: #b7e4c7; box-shadow: inset 4px 0 0 #2f9e44; }
      #${MODAL_ID} .execute-tool { border-color: #99d6ff; box-shadow: inset 4px 0 0 #228be6; }
  ...
  function renderTool(index, toolUse, resultByToolId) {
    ...
    return `
      <div class="item tool ${toolClass(toolUse.name)}">
        ...
        ${input.articleUrl ? `<div><strong>Article:</strong> ${esc(input.articleUrl)}</div>` : ''}
        ${input.code ? `<details><summary>Script</summary><pre>${esc(input.code)}</pre></details>` : ''}
        ${response ? `<details><summary>Response preview · ${esc(responseShape(response))} · ${response.length} chars</summary><pre>${esc(response)}</pre></details>` : ''}
      </div>
    `;
  }

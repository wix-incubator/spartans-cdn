(async function installMcpBiSessionBookmarklet() {
  const VERSION = '2026-05-14-dev-1';
...
`;
  ...
  function normalizeResultRows(result) {
    ...
    return rows.map((row) => Object.fromEntries(ROW_COLUMNS.map((column, index) => [column, row[index]])));
  }
  ...
  function renderToolCall(row, index) {
    ...
    return `<article class="call ${toolClass}">
      ...
      ${row.error_body ? renderErrorBlock(row.error_body) : ''}
    </article>`;
  }
  ...
  function getResponseInfo(row) {
    ...
    const preview = additionalFields?.resultPreview;

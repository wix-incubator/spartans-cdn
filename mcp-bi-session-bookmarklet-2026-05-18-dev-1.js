(async function installMcpBiSessionBookmarklet() {
  const VERSION = '2026-05-18-dev-1';
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      #${MODAL_ID} button.conv-btn { background: #f0fdf9; color: #0e9384; border-color: #0e9384; }
      ...
      #${MODAL_ID} .session-boundary { padding: 14px 0 4px; }
      #${MODAL_ID} .session-boundary hr { border: 0; border-top: 2px dashed #d0d5dd; margin: 0 0 8px; }
      #${MODAL_ID} .boundary-label { color: #667085; font-size: 12px; font-style: italic; }
  ...
  function ensureUi() {
    ...
    if (!modal) {
        ...
        <div class="panel">
          <div class="topbar">
            ...
            <div class="actions">
              ...
              <button class="conv-btn" style="display:none">Expand conv</button>
      ...
      modal.querySelector('.conv-btn').addEventListener('click', () => {
        if (state.nearbySessionIds.length > 1) {
          loadConversation(state.nearbySessionIds);
        }
      });
  ...
  function showExpandButton(sessionCount) {
    const btn = document.querySelector(`#${MODAL_ID} .conv-btn`);
    if (!btn) return;
    btn.textContent = `Expand conv (${sessionCount} sessions)`;
    btn.style.display = '';
  }
...
`;
  ...
  function buildMultiSessionSql(sessionIds) {
    const inList = sessionIds.map((id) => `'${id.replaceAll("'", "''")}'`).join(', ');
    return `
SELECT
  date_created,
  uuid,
...
`;
  ...
  function buildConvDiscoverySql(uuid, clientName, firstSeen, lastSeen, gapMinutes) {
    const escUuid = uuid.replaceAll("'", "''");
    const windowStart = adjustTimestamp(firstSeen, -gapMinutes);
    const windowEnd = adjustTimestamp(lastSeen, gapMinutes);
    const clientFilter = clientName
      ? `AND client_name = '${clientName.replaceAll("'", "''")}'`
...
WHERE evid = 1607
  AND uuid = '${escUuid}'
  ${clientFilter}
  AND date_created BETWEEN TIMESTAMP '${windowStart}' AND TIMESTAMP '${windowEnd}'
GROUP BY session_id
ORDER BY min(date_created) ASC
`;
  }

  function adjustTimestamp(ts, deltaMinutes) {
    if (!ts) return ts;
    const date = new Date(String(ts).replace(' ', 'T').replace(/(\.\d+)?Z?$/, 'Z'));
    date.setTime(date.getTime() + deltaMinutes * 60000);
    return date.toISOString().replace('T', ' ').slice(0, 19);
  }

  async function discoverAdjacentSessions(uuid, clientName, firstSeen, lastSeen) {
    const sql = buildConvDiscoverySql(uuid, clientName, firstSeen, lastSeen, CONV_GAP_MINUTES);
    const queryResult = await runQuixQuery(sql, { limit: 50 });
    const rawRows = queryResult.result?.data?.rows || [];
    return rawRows
      .filter((row) => Array.isArray(row) && row[0] && row[0] !== 'session_id')
      .map((row) => ({ session_id: row[0], first_seen: row[1], last_seen: row[2], call_count: row[3] }));
  }

  async function loadSession(sessionId) {
    hideExpandButton();
    state.nearbySessionIds = [];
    ...
    try {
      ...
      sessionExport = buildSessionExport(sessionId, rows, result);
    ...
    if (sessionExport) {
      const { uuid, client_name, first_seen, last_seen } = sessionExport.session;
      if (uuid && first_seen) {
        discoverAdjacentSessions(uuid, client_name, first_seen, last_seen)
          .then((nearby) => {
            const others = nearby.filter((s) => s.session_id !== sessionId);
            if (others.length > 0) {
              const allIds = nearby.map((s) => s.session_id);
              state.nearbySessionIds = allIds;
              showExpandButton(allIds.length);
            }
          })
          .catch((err) => console.warn('Conv discovery failed:', err));
      }
    }
  }

  async function loadConversation(sessionIds) {
    hideExpandButton();
    state.nearbySessionIds = [];
    ...
    try {
      assertQuixPage();
      setLoading(true);
      setContent(`<div class="status">Loading conversation (${sessionIds.length} sessions) from Quix...</div>`);

      const query = buildMultiSessionSql(sessionIds);
      const result = await runQuixQuery(query, { limit: 2000 });
      const rows = normalizeResultRows(result.result);
      const convExport = buildConversationExport(sessionIds, rows, result);
      state.lastExport = convExport;
      setContent(renderSession(convExport));
    } catch (error) {
      console.error(error);
      renderError(error);
    } finally {
  ...
  function buildConversationExport(sessionIds, rows, querybookResult) {
    const rowsBySession = {};
    for (const row of rows) {
      if (!rowsBySession[row.session_id]) rowsBySession[row.session_id] = [];
      rowsBySession[row.session_id].push(row);
    }

    const orderedIds = sessionIds
      .filter((id) => rowsBySession[id]?.length)
      .sort((a, b) => {
        const aTime = rowsBySession[a][0]?.date_created || '';
        const bTime = rowsBySession[b][0]?.date_created || '';
    ...
    for (let i = 0; i < orderedIds.length; i += 1) {
      if (i > 0) {
        const prevLast = rowsBySession[orderedIds[i - 1]].at(-1)?.date_created;
        const currFirst = rowsBySession[orderedIds[i]][0]?.date_created;
        const gapMinutes = prevLast && currFirst
          ? Math.round(
              (new Date(String(currFirst).replace(' ', 'T').replace(/Z?$/, 'Z')) -
               new Date(String(prevLast).replace(' ', 'T').replace(/Z?$/, 'Z'))) / 60000,
            )
          : 0;
        mergedRows.push({ _session_boundary: true, session_id: orderedIds[i], _gap_minutes: gapMinutes });
      }
      mergedRows.push(...rowsBySession[orderedIds[i]]);
    }

    const toolCounts = countBy(rows.map((r) => r.tool_name));
    const errors = rows.filter((r) => r.execution_result === 'Error').length;
    const targetToolCalls = rows.filter((r) => ['ExecuteWixAPI', 'CallWixSiteAPI'].includes(r.tool_name)).length;
    const payloadChars = rows.reduce(
      (t, r) => t + ['params', 'additional_fields', 'error_body'].reduce((s, f) => s + String(r[f] || '').length, 0),
      0,
    );

    return {
      session: {
        conv_id: orderedIds[0] ?? sessionIds[0],
  ...
  function renderSession(sessionExport) {
    ...
    const isConv = Array.isArray(session.session_ids) && session.session_ids.length > 1;
    const dataRows = rows.filter((r) => !r._session_boundary);
    const toolCounts = countBy(dataRows.map((r) => r.tool_name));

    if (dataRows.length === 0) {
      return `<div class="status">No BI tool-call rows found for session <code>${esc(session.session_id ?? session.conv_id)}</code>.</div>`;
    }

    const idBlock = isConv
      ? `<div><span>Conversation</span><strong>${esc(session.conv_id)}</strong></div>
         <div><span>Sessions</span><strong>${session.session_ids.length} (${session.session_ids.map((id) => id.slice(0, 8)).join(', ')})</strong></div>`
    ...
    let callIndex = 0;
    const callsHtml = rows.map((row) => {
      if (row._session_boundary) {
        return `<div class="session-boundary">
          <hr>
          <div class="boundary-label">Reconnect — new session <code>${esc(row.session_id.slice(0, 8))}</code> after ${esc(row._gap_minutes)} min gap</div>
        </div>`;
      }
      callIndex += 1;
      return renderToolCall(row, callIndex);
    }).join('\n');

    return `${kvHtml}<section>${callsHtml}</section>`;
  }
  ...
  function renderToolCall(row, index) {
    ...
    return `<article class="call ${toolClass}">
      ...
      ${response ? renderResponse(response) : renderMissingResponse(row)}
  ...
  function renderMissingResponse(row) {
    if (row.execution_result !== 'Success' && !hasVisibleErrorBody(row.error_body)) {
      return '';
    }

    return `<details class="response">
      <summary>Response not captured in BI telemetry</summary>
      <div class="meta">This row has no <code>result</code> or <code>additional_fields.resultPreview</code> payload in <code>events.dbo.users_39</code>.</div>
    </details>`;
  }
